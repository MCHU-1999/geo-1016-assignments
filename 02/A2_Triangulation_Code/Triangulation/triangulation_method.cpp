...
private:
    ...
    std::vector<Vector2D> points_0_, points_1_;  // Observed 2D point in first and second image
public:
    TriangulationObjective(
        const Matrix34& P1, const Matrix34& P2,
        const std::vector<Vector2D>& points_0, const std::vector<Vector2D>& points_1
    ) : Objective_LM(4 * points_0.size(), 3 * points_0.size()), // 4N functions, 3N variables
        P1_(P1), P2_(P2), points_0_(points_0), points_1_(points_1) {}
...
bool Triangulation::triangulation(
    ...
    try {
        ...
        bool refinement_success = refine_all_points(M0, M1, points_0, points_1, points_3d);
